...
const common = {
  ...
  state: {
    ...
    isShade: false,
    screen: -1,
    panelTimeout: 'pntimeout',
    visiterTimeout: 'vstimeout',
    parkingTimeout: 'patimeout',
    ...
    themeName: getStore({ name: 'themeName' }) || 'theme-yutong',
    lockPasswd: getStore({ name: 'lockPasswd' }) || '',
    website: website
  },
  actions: {},
  mutations: {
    SET_PANELTIMEOUT: (state, timeout) => {
      state.panelTimeout = timeout
    },
    SET_VISITERTIMEOUT: (state, timeout) => {
      state.visiterTimeout = timeout
    },
    SET_PARKINGTIMEOUT: (state, timeout) => {
      state.parkingTimeout = timeout
    },
