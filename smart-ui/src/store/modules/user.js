...
import {getSecurityEncodeKey} from '@/util/security'
  ...
  actions: {
    ...
    LoginByUsername({commit}, userInfo) {
      const user = encryption({
        data: userInfo,
        key: getSecurityEncodeKey(),
        param: ['password']
      })
      return new Promise((resolve, reject) => {
        loginByUsername(user.username, user.password, user.code, user.randomStr).then(response => {
          // Each login re-evaluates password strength; drop any stale weak-pwd flag (CR-C14-002).
          localStorage.removeItem('isStrongPwd')
