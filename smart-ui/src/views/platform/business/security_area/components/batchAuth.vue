<template>
  ...
  >
    <div class="cks">
      <el-input placeholder="输入关键字进行过滤" clearable v-model="name" size="mini" style="width: 310px;margin-bottom:4px">
        <el-button type="info" slot="append" icon="el-icon-search" @click="getAuthList"></el-button>
      </el-input>
      <el-transfer
...
export default {
  ...
  methods: {
    ...
    initSelectIds() {
      this.myIds = []
      if (this.itemObj.authList && this.itemObj.authList.length > 0) {
        this.itemObj.authList.forEach((el) => {
    ...
    async addSubmit() {
      ...
      this.itemObj.authList = temp2
      // console.log(this.itemObj.myIds, this.itemObj.authList  )
      this.close()
    },
    /**
     * 获取权限列表
    ...
    async getAuthList() {
      this.authList = []
      let list = []
      const res = await bsSecurityAreaApi.getPersonAuthes(this.parkId, {
