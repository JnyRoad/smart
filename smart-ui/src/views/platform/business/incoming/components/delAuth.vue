  ...
  >
    <div class="cks">
      <el-checkbox-group v-model="authIds">
        <el-checkbox v-for="(item, index) in authLists" :label="item.authId" :key="index">
          {{item.authName}}
        </el-checkbox>
      </el-checkbox-group>
      <div class="noData" v-if="!authLists || authLists.length===0" style="height: 45px;">当前无内容</div>
    </div>
    <div slot="footer">
  ...
  methods: {
    ...
    async addSubmit(){
      ...
      this.authIds.forEach(el1=>{
        ...
        let index = this.authLists.indexOf(temp[0])
        this.authLists.splice(index, 1)
      })
      this.itemObj.authLists = this.authLists
      this.close()
    },
    /**
     * 获取权限列表
     */
    async getAuthList() {
      this.authLists = this.itemObj.authLists
    },
    cancel() {
      this.authIds = []
