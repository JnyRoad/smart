      ...
      <section class="my-basic-inner">
        <div class="content">
          <el-form ref="dataform" :rules="emailRule" :inline="false" class="dot-form" :model="dataform" label-width="100px">
            <el-form-item label="保安放行时，是否需要上传照片" label-width="240px">
              <el-switch v-model="dataform.isUploadImg" active-value="0" inactive-value="1" active-color="#13ce66"></el-switch>
            </el-form-item>
                    ...
                    <div>
                      <div class="row2" v-for="(itme_t, index_t) in item.conditions" :key="'t'+index_t">
                        ...
                        <el-select v-model="itme_t.conditionType" @change="conditionTypeChange(itme_t)">
                          <el-option label="物品类型" :value="3"></el-option>
                        ...
                        <template>
                          ...
                          <!-- 物品类型 -->
                          <el-select v-model="itme_t.compareValue"  v-if="itme_t.conditionType===3">
                            <el-option v-for="(good_item, good_index) in goodArr" :label="good_item.desc" :value="good_item.code" :key="good_index"></el-option>
                          </el-select>
...
<script>
import { getStaffDetail, editObj, getObj, getMsg, getGoodType } from './_service'
export default {
  data() {
    return {
      loading: false,
  ...
  created() {
    ...
    this.dataform.isUploadImg = this.$route.query.isUploadImg+''
  },
  mounted: function () {},
  computed: {
  },
  methods: {
    ...
    async saveInfo(){
      ...
      obj.editApprovalNode.forEach(el=>{
        ...
        if(el.approvalPersons && el.approvalPersons.length == 0 && el.isExistApprover != 2){
      ...
      let queryData = {
        approvalId: this.id,
        isUploadImg: this.dataform.isUploadImg
      }
