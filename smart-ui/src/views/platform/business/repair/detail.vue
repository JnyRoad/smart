  ...
  <div class="my-basic-container visitor">
    <el-scrollbar class="my-scrollbar" :native="false">
      <section class="my-basic-inner">
        <div class="content">
          <el-form ref="dataform" :rules="emailRule" :inline="false" class="dot-form" :model="dataform" label-width="100px">
            <div class="node1" v-for="(item, index) in dataform.editApprovalNode" :key="index">
              <div class="row1">
                <div class="btn">
                  <el-button type="text" @click="delNode(index)" v-if="dataform.editApprovalNode.length!==1">删除条件</el-button>
                </div>
                  ...
                  <el-form-item label="触发条件" prop="type">
                    <div>
                      <div class="row2" v-for="(itme_t, index_t) in item.conditions" :key="'t'+index_t">
                        ...
                        <div class="btns">
                          <el-button type="text" icon="el-icon-plus" @click="addCondition(item.conditions)" v-if="item.conditions.length ===0 || index_t === item.conditions.length-1"></el-button>
                        ...
                        <el-select v-model="itme_t.conditionType" @change="conditionTypeChange(itme_t)">
                          <el-option label="维修区域" :value="1"></el-option>
                          <el-option label="维修类型" :value="2"></el-option>
                          <el-option label="楼栋名称" :value="4"></el-option>
                        </el-select>
                        <template>
                          ...
                          <!-- 维修范围 -->
                          <el-select v-model="itme_t.compareValue"  v-if="itme_t.conditionType===1">
                            <el-option v-for="(range_item, range_index) in repairRange" :label="range_item.desc" :value="range_item.code" :key="range_index"></el-option>
                          </el-select>
                          <!-- 维修类别 -->
                          <el-select v-model="itme_t.compareValue"  v-if="itme_t.conditionType===2">
                            <el-option v-for="(type_item, type_index) in repairType" :label="type_item.desc" :value="type_item.code" :key="type_index"></el-option>
                          </el-select>
                          <!-- 楼栋 @doChange="doDormChange"-->
                          <dormSelect
                            v-if="itme_t.conditionType===4"
                            :parkId="parkId"
                            v-model="itme_t.compareValue"
                          ></dormSelect>
                  ...
                  <el-form-item label="处理人设置">
                        ...
                        <tr v-for="(person_item, person_index) in item.approvalPersons" :key="'person'+person_index">
                          ...
                          <td>
                            <el-form-item label prop="">
                              <el-input type="text"  @keypress.native.enter="getStaffDetail(person_item, person_index, item.approvalPersons)" v-model="person_item.approverBadge" placeholder="输入后回车，查询姓名"></el-input>
                            </el-form-item>
                          </td>
                          <td>
                            <el-form-item label prop="">
                    ...
                    <div>
                      <el-radio-group v-model="item.passRule">
                        ...
                        <el-radio :label="2">审批人任其一通过，进入下个流程</el-radio>
                      </el-radio-group>
                    </div> -->
                  </el-form-item>
              ...
              <el-button type="text" @click="addNode" v-if="index === dataform.editApprovalNode.length-1">添加条件</el-button>
...
<script>
import { getStaffDetail, editObj, getObj, getMsg, getRepairRange, getRepairsType } from '../release/_service'
export default {
  data() {
    return {
      loading: false,
  ...
  methods: {
        ...
        if(res.data.code===0 && res.data.data.length > 0){
          ...
          this.dataform.editApprovalNode.forEach(el=>{
            ...
            if(!el.approvalPersons || el.approvalPersons.length==0){
              el.approvalPersons = [ this.getPerson() ]
            }
    ...
    async saveInfo(){
      ...
      try {
        const res = await editObj(obj.editApprovalNode)
        if(res.data.code===0){
          this.$notify({
            title: '成功',
            message: '修改成功',
    ...
    // 维修范围
    async getRepairRange(){
      const res = await getRepairRange()
      this.repairRange = res.data.data
    },
    // 维修类别
    async getRepairsType(){
      const res = await getRepairsType()
