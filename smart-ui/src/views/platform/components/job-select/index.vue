...
const getList = function(depId) {
  return request({
    url: `${baseUrl}/getJob/${depId}`,
    method: "GET"
  });
};
export default {
  ...
  mounted() {
    ...
    this.depId && this.getDataList(this.depId);
  },
  watch: {
    value(val, oldval) {
      this.currVal = val;
  ...
  methods: {
    async getDataList(depId) {
      let res = await getList(depId);
      this.dataList = [];
      res.data.data.forEach(element => {
        this.dataList.push({ value: element.jobid, label: element.jobname });
      });
    },
    handleChange(id) {
