      ...
      <section class="my-basic-inner">
        ...
        >
          <template slot-scope="scope" slot="startDate">
            <span>{{ scope.row.startDate | dateFormat2}}</span>
          </template>
          <template slot-scope="scope" slot="endDate">
            <span>{{scope.row.endDate | dateFormat2}}</span>
          </template>
...
<script>
import { fetchList } from "@/api/platform/work/ask_leave";
import { tableOption } from "@/const/crud/platform/work/ask_leave";
...
export default {
  ...
  methods: {
    ...
    handleDetail(row, index) {
      const src = `/platform/work/askLeave/detail/${row.recordId}`;
      this.$router.push({
        path: src,
        query: {
