    ...
    <el-scrollbar class="my-scrollbar" :native="false">
      <section class="my-basic-inner">
        <div class="top-menu">
          ...
          <span style="font-size: 16px;margin-left: 10px;line-height: 30px;">当前设备名称：{{eleMeterName}}</span>
          <div class="top-right">
...
export default {
  ...
  created: function () {
    this.eleMeterId = this.$route.params.id
    this.eleMeterName = this.$route.query.name
  ...
  methods: {
    goBack() {
      const path = `/platform/device/electricManage`
      this.$router.push({
        path: path,
        query: {
