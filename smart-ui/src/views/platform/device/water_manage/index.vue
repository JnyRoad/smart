<!--设备管理，闸机管理  -->
<template>
  <div class="my-basic-container device water">
      ...
      <section class="my-basic-inner">
        <div class="top-menu">
          <div class="top-right">
            ...
            <el-button type="primary" icon="el-icon-plus" @click="addFormVisible = true">添加水表</el-button>
            <el-button type="primary" @click="supplierImportDialogShow">导入水表</el-button>
            <el-button type="primary" :loading="exportLoading" @click="export2Excel" icon>导出水表</el-button>
            ...
            <template v-for="(item, key, index) in datalist">
              <div class="device-outer" :key="index">
                <dl class="device-item clear">
                  ...
                  <dt v-if="item.largeClass === '0'" style="background-image: url('/img/device_05.png')"></dt>
                  <dt v-if="item.largeClass === '1'" style="background-image: url('/img/device_08.png')"></dt>
                  <dt v-if="item.largeClass === '2'" style="background-image: url('/img/device_09.png')"></dt>
                  <dt v-if="item.largeClass === '3'" style="background-image: url('/img/device_10.png')"></dt>
                  <dt v-if="item.largeClass === '4'" style="background-image: url('/img/device_11.png')"></dt>
                  <dd>
                    ...
                    <div class="info-tbl">
                      ...
                      <div class="tbl_row">
                        <div class="tbl_cell tbl_cell_label">用户大类</div>
                        <div class="tbl_cell" :title="item.largeClassDesc">{{ item.largeClassDesc || '-' }}</div>
                      </div>
                      <div class="tbl_row">
                      ...
                      <!-- <div class="tbl_row">
                        <div class="tbl_cell tbl_cell_label">阀门控制</div>
                        <div class="tbl_cell">
                          <el-switch
        ...
        <el-dialog :title="addTitle" :close-on-click-modal="false" width="700px" :visible.sync="addFormVisible" @close="closeDialog">
          <el-form :rules="addRules" ref="addForm" :model="addForm" label-width="120px">
            ...
            <el-form-item label="下行通道" prop="port">
            ...
            <el-form-item label="用户大类" prop="largeClass">
              <el-select v-model="addForm.largeClass" placeholder="用户大类" clearable>
                <template v-for="(item, index) in largeClassList">
                  <el-option :label="item.label" :value="item.value" :key="index"></el-option>
                </template>
              </el-select>
            </el-form-item>
    ...
    <!-- 批量导入 -->
    <el-dialog title="导入水表" width="500px" @close="resetUploadSupplierForm('uploadSupplierForm')" :visible.sync="supplierImportDialog.visible">
      <el-form ref="uploadSupplierForm" :model="supplierImportDialog.formData" label-width="120px">
        <el-form-item label="水表模板">
          <a class="linkT" href="/resource/water-temp.xls" target="__blank">下载模板</a>
        </el-form-item>
        <el-form-item label="导入文件" prop="myFileSupplier">
          <input
            type="file"
...
<script>
import { concentratorList, getTagList, addObj, fetchList, batchDelete, putObj,changeObj, putValve, supplierImport, flushProgress, meterRead, reDownload } from './_service'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      addRules: {
        ...
        port: [{ required: true, message: '请选择下行通道', trigger: 'change' }],
        largeClass: [{ required: true, message: '请选择用户大类', trigger: 'change' }],
        placeType: [{ required: true, message: '请选择区域类型', trigger: 'change' }],
      ...
      isEdit: false,
      portList: [
        { label: 'M-BUS-1', value: '2' },
        { label: 'M-BUS-2', value: '3' },
        { label: 'M-BUS-3', value: '4' },
        { label: 'M-BUS-4', value: '5' }
      ],
      largeClassList: [
        { label: '冷水水表', value: '0' },
        { label: '热水水表', value: '1' },
  ...
  created() {
    this.$nextTick(() => {
      if (this.$route.query.queryForm != undefined) {
        const queryPage = this.$route.query.queryPage
        const queryForm = this.$route.query.queryForm
  ...
  methods: {
      ...
      reader.onload = () => {
        try {
          ...
          if (res.data.byteLength > 0) {
            ...
            fileDownload(res.data, '水表错误表格.xls')
          } else {
            this.$message({
              message: '导入成功',
              type: 'success'
    ...
    supplierImportDialogShow() {
      this.supplierImportDialog.visible = true
    },
    uploadSupplier() {
          ...
          if (this.addForm.id) {
            ...
            switch (this.addForm.placeType) {
              ...
              case '0':
                ...
                obj['roomName'] = this.addForm.roomName
                break
            }
            if (this.isEdit) {
              //更换设备
    ...
    handleEdit(item, type) {
      ...
      this.addForm = { ...item }
      this.addForm.roomName = this.addForm.dormitoryName + this.addForm.roomName
    ...
    handleDel: function (row) {
      ...
      const elm = this.$createElement
      this.$msgbox({
        message: elm('p', { attrs: { class: 'smallp' } }, [elm('i', { attrs: { class: 'smallInfo delInfo' } }, ''), elm('span', null, '确认删除该水表信息？ ')]),
        showCancelButton: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        customClass: 'small_dialog',
      ...
      require.ensure([], () => {
        ...
        fetchList(params)
          .then(response => {
            ...
            export_json_to_excel(tHeader, data, "水表信息");
     ...
     */
    logsList(item) {
      const src = `/platform/device/waterManage/log/${item.id}`
      this.$router.push({
        path: src,
        query: {
          queryPage: this.page,
     ...
     */
    permittedList(item) {
      const src = `/platform/device/waterManage/record/${item.id}`
      this.$router.push({
        path: src,
        query: {
          queryPage: this.page,
    ...
    fileChangeSupplier(e) {
      ...
      if (e.target.files.length > 0) {
        ...
        _this.supplierImportDialog.formData.myFileSupplier = file
      }
    }
  }
}
