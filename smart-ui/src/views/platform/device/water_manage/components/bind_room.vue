...
<script>
import { allList, fetchRoomList } from '../_service.js'
export default {
  data() {
    return {
      currVisible: false,
  ...
  methods: {
    ...
    formSumit() {
      if(Object.keys(this.curRoom).length === 0){
        this.$message.error('请在右侧选择绑定的房间')
        return
      }
      this.cancel()
      this.$emit('done', this.curRoom)
    },
