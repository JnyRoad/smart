      ...
      <section class="my-basic-inner">
        ...
        <avue-crud ref="crud" :page="page" :data="tableData" :table-loading="tableLoading" :option="tableOption" @size-change="sizeChange" @current-change="currentChange">
          <template slot-scope="scope" slot="menu">
            <el-button type="text" @click="downloadFile(scope.row)">下载档案</el-button>
...
<script>
import { concentratorList, addConcentrator, updateConcentrator, delConcentrator, downloadFile} from './_service'
...
export default {
  ...
  methods: {
    ...
    async downloadFile(row){ //下载档案
      const res =  downloadFile(row.id)
    },
    handleDetail(row) {
      this.dialogVisible = true
    ...
    getList(page, params) {
     ...
     viewDevice(item){
      const src = `/platform/device/electricManage`
      this.$router.push({
        path: src,
        query: {
          concenId: item.id
