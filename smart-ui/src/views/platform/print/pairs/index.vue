...
export default {
  ...
  data() { return { personTypes, records: [], templates: [], page: 1, total: 0, busy: false, error: '', message: '', form: null, previewResult: null, pendingMutation: null, pendingArchive: null, pendingRefresh: false, parkGeneration: 0 } },
  computed: {
  ...
  watch: { parkId: { immediate: true, handler() { this.parkGeneration++; this.form = null; this.previewResult = null; this.records = []; this.templates = []; this.total = 0; this.page = 1; this.pendingMutation = null; this.pendingArchive = null; if (this.busy) { this.pendingRefresh = true; return } this.refresh() } }, form(value) { this.$emit('editing-state', !!value) }, busy(value) { this.$emit('busy-state', value); if (!value && this.pendingRefresh) { this.pendingRefresh = false; this.refresh() } } },
  ...
  methods: {
    ...
    async loadList() { const park = this.parkId; if (!park) { this.records = []; this.templates = []; return } const data = await api.listPairs({ parkId: park, current: this.page, size: 20 }); if (park !== this.parkId) return; this.records = recordsOf(data); this.total = Number(data.total || this.records.length) },
    ...
    async beginEdit(row) { return this.run(async () => { const park = this.parkId; const generation = this.parkGeneration; const summaries = []; let current = 1; let total; do { const data = await listTemplates({ parkId: park, printItemType: 'STAFF_CARD', lifecycleStatus: 'ACTIVE', current, size: 100 }); if (generation !== this.parkGeneration || park !== this.parkId) return; const records = recordsOf(data); summaries.push(...records); total = Number(data.total || summaries.length); if (!records.length) break; current++ } while (summaries.length < total); const templates = await Promise.all(summaries.map(item => getTemplate(item.templateId, park))); if (generation !== this.parkGeneration || park !== this.parkId) return; const detail = row ? await api.getPair(row.pairId, park) : null; if (generation !== this.parkGeneration || park !== this.parkId) return; this.templates = templates; this.form = detail ? { ...detail } : { name: '', frontTemplateVersionId: '', backTemplateVersionId: '' }; this.pendingMutation = null }) },
