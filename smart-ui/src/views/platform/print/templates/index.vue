<template>
  <section class="print-page" :aria-busy="busy">
    ...
    <template v-if="!current">
      <div class="print-toolbar"><label>打印物 <select v-model="filter" :disabled="busy" @change="applyFilter"><option value="">全部</option><option value="STAFF_CARD">人员厂牌</option><option value="VISITOR_SLIP">访客凭条</option></select></label><button :disabled="busy || !parkId" @click="refresh">刷新列表</button></div>
    ...
    <section v-if="createVisible" class="print-dialog" role="dialog" aria-label="新建单面模板">
      <h2>新建单面模板</h2><form @submit.prevent="confirmCreate">
        ...
        <label>人员类型 <select v-model="createForm.personType" @change="normalizeCreateClassification"><option v-for="(label, value) in allowedPersonTypes" :key="value" :value="value">{{ label }}</option></select></label>
        ...
        <label v-if="createForm.printItemType === 'VISITOR_SLIP'">分类 <select v-model="createForm.classificationCode"><template v-if="createForm.personType === 'VISITOR'"><option value="VISITOR_NORMAL">普通访客</option><option value="VISITOR_SECURITY">保密访客（必须带码）</option></template><option v-else value="SUPPLIER_DEFAULT">供应商凭条</option></select></label>
...
export default {
  ...
  computed: { allowedPersonTypes() { return this.createForm.printItemType === 'VISITOR_SLIP' ? { VISITOR: '访客', SUPPLIER: '供应商人员' } : { EMPLOYEE: '正式员工', OUTSOURCED: '外包人员', DISPATCHED: '派遣人员' } } },
  ...
  methods: {
    ...
    beginCreate() { this.createForm = { name: '', printItemType: 'STAFF_CARD', personType: 'EMPLOYEE', faceRole: 'FRONT', classificationCode: 'STAFF_DEFAULT' }; this.createVisible = true },
    resetCreateType() { this.createForm.personType = this.createForm.printItemType === 'VISITOR_SLIP' ? 'VISITOR' : 'EMPLOYEE'; this.createForm.faceRole = 'FRONT'; this.normalizeCreateClassification() },
    normalizeCreateClassification() { this.createForm.classificationCode = newTemplateDraft(this.createForm.printItemType, this.createForm.personType, 'FRONT').classificationCode },
    confirmCreate() { const draft = newTemplateDraft(this.createForm.printItemType, this.createForm.personType, this.createForm.faceRole); draft.name = this.createForm.name; if (draft.printItemType === 'VISITOR_SLIP' && draft.personType === 'VISITOR' && ['VISITOR_NORMAL', 'VISITOR_SECURITY'].includes(this.createForm.classificationCode)) draft.classificationCode = this.createForm.classificationCode; this.current = {}; this.draft = draft; this.canvas = canvasFromVersion(draft); this.editorKey++; this.dirty = true; this.createVisible = false },
    async open(id) { return this.run(() => this.loadDetail(id)) },
    async loadDetail(id) { const detail = await api.getTemplate(id, this.parkId); const version = detail.draft || detail.versions.find(item => item.templateVersionId === detail.currentPublishedVersionId); if (!version) throw new Error('模板没有可读取的版本'); const draft = { ...version, pageSpecJson: version.pageSpecJson || {}, name: detail.name, printItemType: detail.printItemType, personType: detail.personType, classificationCode: detail.classificationCode, faceRole: detail.faceRole, sideCount: 1 }; const canvas = await hydrateTemplateResources(draft, { parkId: this.parkId, downloadResource: api.downloadTemplateResource, cache: this.resourceCache }); this.current = detail; this.draft = draft; this.canvas = canvas; this.editorKey++; this.dirty = false },
    ...
    async publish() { return this.run(async () => { const data = { parkId: this.parkId, draftRevision: this.current.draftRevision, draftVersionId: this.current.currentDraftVersionId }; const signature = JSON.stringify({ id: this.current.templateId, data }); if (!this.pendingPublish || this.pendingPublish.signature !== signature) this.pendingPublish = { signature, key: newIdempotencyKey() }; await api.publishTemplate(this.current.templateId, data, this.pendingPublish.key); await this.loadDetail(this.current.templateId); this.pendingPublish = null; this.message = '当前单面模板已发布，已有组合保持原版本'; }) },
