...
<template>
  <div class="my-basic-container staff">
    <el-scrollbar class="my-scrollbar" :native="false">
      <section class="my-basic-inner">
        <div class="top-menu">
          <div class="top-left">
            <span>查询小计</span>
            <span>已签收:{{count.countSign}}</span>
            <span>未签收:{{count.countNotSign}}</span>
          </div>
          <div class="top-right">
    ...
    <el-dialog
      ...
      width="600px">
      <p style="padding: 10px 0 30px">点击确认发送短信，将自动短信通知当前筛选条件下的 <span style="font-weight: bold; font-size: 18px;">{{msgNum}}</span> 位员工进行考勤确认</p>
...
<script>
  import {fetchList, sendMsg, sendMsgNum} from "./salary_sign.js";
  import {getCompTree} from "@/api/platform/_publicService";
  import {tableOption} from "@/const/crud/platform/personnel_manage/salary_sign";
  import {mapGetters} from "vuex";
  import {isArrayFn,getDateMonth, getDatePreMonth} from "@/util/util";
  export default {
    name: "salary_sign",
    data() {
      return {
    ...
    created() {
      this.searchForm.wageDate = getDatePreMonth();
    ...
    methods: {
        ...
        require.ensure([], () => {
          ...
          fetchList(
            ...
            list.forEach(function (item) {
              item.signStatus == 1 ? (item.signStatusDesc = "待签收") : (item.signStatusDesc = "已签收");
            });
            const data = this.formatJson(filterVal, list);
            export_json_to_excel(tHeader, data, `工资签收管理&(${this.searchForm.wageDate})`);
      ...
      handleDetail(row, index) {
        const src = `/platform/personnel_manage/salary_sign/detail/${row.id}`;
        this.$router.push({
          path: src
        });
      },
