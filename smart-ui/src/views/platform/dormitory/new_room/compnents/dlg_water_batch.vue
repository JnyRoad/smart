    ...
    async getDetail(e) {
      ...
      if(!this.validatenull(month) && !this.validatenull(this.floorId)){
        ...
        if(res.data.code==0){
          ...
          res.data.data.forEach(el=>{
            ...
            if(el.commonCates && el.commonCates.length>0){
              obj1 = el.commonCates.find(item => {
                if (item.categoryId === this.itemType) { //根据传值，取热水、冷水、电
                  return item;
                }
              });
            }
            ...
            //获取不重复房间
            let roomObj = rooms.find(rm=>{
              if(rm.roomId === el.roomId){
                return rm
              }
            })
            if(!roomObj){
          ...
          let diffArr = []
          rooms.forEach(el=>{
            let rmItem = this.addForm.records.find(rm=>{
              if(rm.roomId === el.roomId){
                return rm
              }
            })
          ...
          if(diffArr.length>0){
            for(let i = 0; i<diffArr.length; i++){
              this.addForm.records.push({
                meterMonth: diffArr[i].meterMonth,
                meterReadDetailList: [
                  {
                    categoryId: this.itemType,
                    curMonthNum: undefined,
                    preMonthNum: undefined,
                    avgNum: undefined
                  }
                ],
                roomId: diffArr[i].roomId,
                roomName: diffArr[i].roomName,
                statementStatus: diffArr[i].statementStatus
              })
            }
            this.allClosed = false
          }else{
            this.addForm.records.forEach(el=>{
              //有未结算的，就是没有 全部结算完
