...
**Performance Goals**: 只串行化共享证件号的提交；不同证件号不互相阻塞；已存在锁行等待超时返回明确的可重试业务失败；首次锁行 `INSERT` 使用 10 秒 JDBC 语句超时并转换为业务失败。2026-09-02 的正式 Oracle 锁表验收观察到竞争连接约 12.1 秒后取消，故 10 秒不是精确硬上限
...
- 真实数据与 DDL：通过（受限）。实现只提供可审核的表定义；工作区未执行真实 DDL、DML、索引或统计信息操作。经授权的正式库锁表验证只插入随机哈希并全程回滚，未提交业务数据。
- 中文可维护性：通过。新增类、方法、SQL 分支和测试意图均使用中文说明。
- 面向行为的分层验证：通过。先增加会失败的 Mapper/服务测试，再实现最小事务锁与判重逻辑；正式 Oracle 锁表 20 轮并发验收已完成，业务 HTTP 端到端验收仍待部署环境执行。
