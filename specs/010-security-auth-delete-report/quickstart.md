...
- 后端新增回归先复现缺失时间导致 `FAILED`，以及新增跳过结果被参数校验拒绝；修复后 4 个相关测试类共 49 项通过，Maven `package` 成功。
- `SKIPPED_MISSING_TIME` 明确区分缺少判定依据与执行失败，保留权限且继续处理后续人员；真实进出记录存在时仍可正常判定。该结果使用现有结果文本列，无需追加数据库结构变更。
- NORMAL/ISC 任务缺失时保留接口既有 `status=null` 契约，界面按未知显示；修正 DTO 的误导注释并覆盖两来源的空状态及 0–6 状态透传。
- 白名单真实挂载回归使用工号输入、回车查询姓名和添加按钮，校验名单保留且输入清空；同一测试对原组件也通过，未复现“实际输入无法清空”的评审描述。已补齐三项临时字段默认值，移除响应式内部断言；路由测试改为导入父子路由并解析实际懒加载入口。
- 本轮前端全量 Vitest 86 个文件、439 项通过，lint 和生产构建退出码均为 0；lint 仍有仓库 warning，构建使用本地占位地址，未部署。后端差异独立复核未发现阻断问题。
- 从实际 XML 展开共享结果片段后，在内存 SQLite 验证 18 条本园区及 1 条其他园区记录、15 种任务状态组合、新跳过结果、筛选与空范围拒绝；断言通过，不能替代 Oracle 验收。
...
6. 使用目标规模数据查看 Oracle 分页、总数和详情查询的实际执行计划，确认任务关联唯一键以 `LOG_ID` 为首列且索引有效，检查聚合范围及条件下推；测量首页耗时，小于 3 秒才通过性能验收。尚未取得目标计划，不预判 Oracle 一定使用索引或先分页后聚合。
