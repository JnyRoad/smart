...
- [X] T016 按用户授权仅提交本任务文件，推送功能分支并创建面向 main 的 [PR #189](https://github.com/JnyRoad/smart/pull/189)。

FR-009由T014覆盖，FR-010由T015覆盖；T014与升级流程只读调查可并行，T015依赖两者，T016在验证后执行。真实Oracle验证仍由现场验收，不因脚本存在而标记已执行。

## PR #189 评审修复

- [X] T017 先补缺失时间依据与缺失任务详情的后端回归，复现缺陷后实现明确跳过；同步结果筛选和导出口径，复用演练校验及 SQL 结果片段。
- [X] T018 [P] 用真实工号查询与添加流程核对白名单输入清空（原组件也通过，未复现评审所述故障），补齐临时字段默认值；补新跳过结果与未知状态显示，路由测试验证导出对象，移除 Vue 内部实现断言。
- [X] T019 独立复核修复，完成相关测试/构建与 SQL 语义验证，记录实际性能验收限制。
- [X] T020 将本轮修复提交 `8f64a938` 推送到原 PR，已核对远端提交一致；新一轮 CodeRabbit 仍在审查中，未合并或部署。
