...
- **Decision**: 姓名条件使用“主申请人姓名匹配 OR 存在随行人员姓名匹配”；证件号条件
  使用“主访客证件号匹配 OR 存在随行人员证件号匹配”。
- **Rationale**: 2026-09-02 的实库默认列表聚合显示，无条件外连随行表时产生 9,648 行而仅有
  7,756 张申请；移除随行外连后为 7,764 行。相差的 8 行来自仍保留的车辆外连接及不同
  车牌，不能被 <code>SELECT DISTINCT</code> 合并。`EXISTS` 只保证匹配随行人员不会再扩展
  主查询行，不能据此声称所有申请单绝对只有一行。
- **Validation**: 在无多车的可控申请单上分别准备“一位匹配随行人员”和“两位均匹配随行人员”
  的数据，按返回记录的申请单 <code>ID</code> 分组比较计数，两组应相同且均为 1；若业务数据有
  多车，先记录无随行条件下该 <code>ID</code> 的既有行数，再断言增加匹配随行人员后该计数不变。
  <code>SmtAdmittanceApplyMapperXmlTest</code> 仅验证 MyBatis 生成的 SQL 和绑定参数，不能替代此项
  真实结果基数验收。
