<template>
  <div class="spring_falling__message_box_btn_wraper">
    <div
      class="spring_falling__message_box_btn_top_line"
      :style="{
    ...
    <div class="spring_falling__message_box_btn_box">
      <div
        class="spring_falling__message_box_btn"
        v-for="(item, index) in btnArr"
        :key="index"
        :data-index="index"
        :style="{color: item.color}"
        @click.stop.prevent="action($event,!index)"
      >{{ item.text }}</div>
    </div>
    <div
      class="spring_falling__message_box_btn_center_line"
      v-if="btnArr.length === 2"
      :style="{
            background: borderColor
        }"
    ></div>
