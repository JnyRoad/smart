      ...
      <cube-scroll ref="scroll" :options="options" :data="dataList" @pulling-down="onPullingDown" @pulling-up="onPullingUp">
        <div class="item" @click="toDetail(item)" v-for="(item, index) in dataList" :key="index">
          <div class="item-title">
            <div class="item-title_left">{{item.name}}提交的园区报修</div>
          ...
          <div class="item-info">
            <div class="item-info_row">
              ...
              <div class="item-info_value -ellipsis">{{item.rangeTypeDesc}}</div>
            </div>
            <div class="item-info_row">
              <div class="item-info_label">
                <tce-label-justify label="维修类别:"></tce-label-justify>
              </div>
              <div class="item-info_value -ellipsis">{{item.repairTypeDesc}}</div>
            </div>
            <div class="item-info_row">
              <div class="item-info_label">
                <tce-label-justify label="所在楼栋:"></tce-label-justify>
              </div>
              <div class="item-info_value -ellipsis">{{item.dormitoryName}}</div>
            </div>
            <div class="item-info_row">
              <div class="item-info_label">
                <tce-label-justify label="故障描述:"></tce-label-justify>
              </div>
              <div class="item-info_value -ellipsis">{{item.faultDesc}}</div>
            </div>
            <div class="item-info_row">
              <div class="item-info_label">
...
import { getRepairList } from '@/services/dormRepairs'
...
export default {
  ...
  methods: {
    ...
    toDetail(item) {
      this.$router.push({
        path: '/xuchang/dormRepairs/detail',
        query: {
          id: item.id
        }
      })
