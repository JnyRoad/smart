...
<template>
  <div class="page3">
    <div class="page3-list" v-if="releaseGoodsInfo && releaseGoodsInfo.length>0">
      <div class="top-tip">放行物品（{{releaseGoodsInfo.length}}项）</div>
      <div class="item" v-for="(item, index) in releaseGoodsInfo" :key="index" @click="detail(item)">
...
export default {
  ...
  methods: {
    ...
    detail(item) {
      this.$router.push({
        path: '/xuchang/goodReleaseWork/detailGoods',
        query: {
   ...
   */
  created() {
    this.releaseGoodsInfo = JSON.parse(this.$route.query.list)
  },
  /**
   * 生命周期 mounted
   */
