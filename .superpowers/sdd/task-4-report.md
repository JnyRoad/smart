...
- 详情页补充独立状态机竞态覆盖：销毁后旧成功不写进度/不设定时器，切换批次后旧成功不影响新轮询，切换批次后旧失败不停止新轮询；覆盖后 `pnpm test` 通过，72 个测试文件、387 个测试。
- 复审加严详情页时序：先完成新批次并保留其进度/timer，再处理旧成功或旧失败；两者均不能覆盖、清理或提示新批次。Axios 另覆盖 HTTP 201 与 204，证明完整 2xx 兼容范围；覆盖后 `pnpm test` 通过，72 个测试文件、389 个测试。
