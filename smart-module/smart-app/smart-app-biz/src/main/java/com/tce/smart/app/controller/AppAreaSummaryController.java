...
@RestController
@AllArgsConstructor
@RequestMapping("/appsummary")
public class AppAreaSummaryController extends BaseController {
	...
	public Result getAppSubjectOnline(Page page, @PathVariable(required = false)Integer parkId) {
		...
		String catalogCode = SubjectCatalog.PARK_INTRODUCE.type();
		IPage<AppSubject> iPage = appSubjectService.getAppSubjectPage(page, publishFlag, catalogCode, parkId);
	...
	public Result addSubject(@RequestBody AddAppSubjectAo addAppSubjectAo) {
		Integer subjectId = appSubjectService.subjectInsert(addAppSubjectAo, SubjectCatalog.PARK_INTRODUCE.type());
