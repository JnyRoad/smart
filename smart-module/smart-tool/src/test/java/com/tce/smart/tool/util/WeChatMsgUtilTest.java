 ...
 * 旧 sendMsg 签名必须保持行为逐字节不变（全系统 18 个调用点依赖）。
 */
public class WeChatMsgUtilTest {
	...
	/** 默认模板名常量必须保持「访客出入园提醒」——18 个旧调用点靠它维持现状 */
	@Test
	...
	/**
	 * body 为空时：validateParameters 在发 HTTP 前抛 IllegalArgumentException，
	 * 被 sendTemplateMsg 的 catch 吞掉返回 Boolean.FALSE（锁定存量「失败/异常一律返回
	 * false 不抛出」契约；因异常发生在 HTTP 之前，不会真发请求）。
	 */
	@Test
	public void sendTemplateMsg_emptyBody_returnsFalseWithoutThrowing() {
		Boolean result = WeChatMsgUtil.sendTemplateMsg("任意模板", "张三", "", null, null, null);

		assertEquals(Boolean.FALSE, result);
	}

	/**
	 * 委托语义等价验证（不发 HTTP）：旧 sendMsg 走默认模板、且 loginName 同时作为
	 * thing18 展示值。用 buildMessageData + buildRequestParameter 复现 sendMsg 的入参
	 * 组合，断言 thing18=工号、templateName=默认模板。
	 */
	@Test
	public void sendMsg_delegation_usesDefaultTemplateAndLoginNameAsDisplay() {
		JSONObject data = WeChatMsgUtil.buildMessageData("8056297", "正文");
		JSONObject param = WeChatMsgUtil.buildRequestParameter(
				WeChatMsgUtil.DEFAULT_TEMPLATE_NAME, "8056297", null, null, data);

		assertEquals("8056297", data.getJSONObject("thing18").getStr("value"));
		assertEquals(WeChatMsgUtil.DEFAULT_TEMPLATE_NAME, param.getStr("templateName"));
	}
}
