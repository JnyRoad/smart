...
public class NettyServer {
	...
	public void start(Integer port) {
		...
		try {
			...
			// I/O处理类,主要用于网络I/O事件，记录日志，编码、解码消息
			serverBootstrap = serverBootstrap.childHandler(new ChannelInitializer<Channel>() {

				@Override
				protected void initChannel(Channel channel) {
					channel.pipeline().addLast(new NettyMessageEncoder());
					// 处理客户端的请求的数据，并且进行响应
					channel.pipeline().addLast(new NettyServerHandler(serverHandler));
				}
			});
