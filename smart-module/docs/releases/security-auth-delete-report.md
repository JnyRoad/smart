...
任务关联唯一约束 `UK_SEC_AUTH_DELETE_TASK (LOG_ID, TASK_SOURCE, TASK_ID)` 的索引以 `LOG_ID` 为首列。上线前核对目标 schema 中该约束对应索引的实际列顺序和有效状态；同时保留有代表性数据量、园区/时间筛选及结果筛选下的实际分页、总数查询和任务详情执行计划。主查询包含任务聚合子查询，必须检查 Oracle 是否下推条件、是否对全部历史任务聚合，以及 `LOG_ID` 索引的实际访问方式；若出现全历史扫描导致超时，先处理计划问题再启用报表，不能仅凭索引存在或本地 SQLite 检查宣称性能达标。
