...
errors << "root snapshotRepository id must be maven-snapshots" unless text_at(root_doc, "/project/distributionManagement/snapshotRepository/id") == "maven-snapshots"
errors << "root snapshotRepository url must point to maven-snapshots" unless text_at(root_doc, "/project/distributionManagement/snapshotRepository/url") == NEXUS_SNAPSHOTS_URL
errors << "root flatten-maven-plugin must be configured in build/plugins" unless text_at(root_doc, "/project/build/plugins/plugin[artifactId='flatten-maven-plugin']/artifactId") == "flatten-maven-plugin"
...
POM_PATHS.each do |path|
  ...
  errors << "#{relative} still contains #{FIXED_VERSION}" if xml.include?(FIXED_VERSION)
end
