...
## 六、现场验收（许昌，第 3 步部署完成后执行；全部通过才进入第 4 步观察期）

> 对应实施计划 Task 11 与 spec §5「人工验收」。每项记录执行时间与结果留档。

| # | 验收项 | 操作 | 预期 | 失败排查 |
|---|---|---|---|---|
| 6.1 | 端到端落盘与打印 | 提一笔测试入厂申请（带人脸照片）→ 审批通过 | ≤2 个拉取周期（默认 ≤60s）内照片出现在 `D:\visitor\{photoId}.png`，b-PAC 打印页面正常打印 | FileReceiver 日志按第 4.1 步关键字排查（token/清单/下载哪一环） |
| 6.2 | 状态跟随 ISC 真实结果 | 观察 6.1 测试单在管理后台/H5 的状态 | ISC 全部下发成功后由「已下发」变「下发成功」；H5 自查显示对应终态 | smart-schedule 日志搜 `AdmittanceDispatchAggregator` 聚合回写记录 |
| 6.3 | 真失败暴露且不被自动补偿重建 | 构造单人测试单，将其目标设备全部离线/禁用后审批通过，等 ISC 重试耗尽 | 状态变「下发失败」；观察 ≥2 个补偿周期，smart-platform 日志**无**该单的补偿认领记录（isc_submit_batch 非空天然隔离） | 若被补偿重建：核查补偿查询的 `isc_submit_batch IS NULL` 谓词是否生效 |
| 6.4 | 重新下发批次化 | 对 6.3 的失败单恢复设备后点「重新下发」 | 旧批次未终态任务状态变 CANCEL（查 `smt_isc_device_task` 按 apply_id+旧 batch_id）；新批次任务生成；最终状态变「下发成功」 | 对照 `repeatVisitorDeviceAuth` 日志与任务表批次号 |
| 6.5 | 断网自愈 | 审批通过一笔新测试单后立即拔掉 FileReceiver 机器网线 5 分钟再恢复 | 恢复后 ≤2 个拉取周期内缺失照片自动补齐，期间拉取轮仅记 ERROR 不崩溃、恢复后无需重启程序 | FileReceiver 日志确认「拉取待处理清单失败」→恢复后转成功 |
| 6.6 | 过期清理双条件 | 在 `D:\visitor` 手工放一个 mtime 早于 retention-days 的假照片文件（文件名用不存在的 UUID）+ 确认一个 pending 中的真实照片同样超期（可调小 retention-days 临时验证） | 次日 03:00 后（或临时改 cron 触发）：假照片被删除，pending 中的照片**保留**；孤儿 `.tmp` 超 1 天的一并清理 | PhotoCleanupTask 日志逐条核对删除记录 |
| 6.7 | 关闭推送后回归 | 完成第 4 步关开关后再提一笔测试单 | 照片仅经拉取到达（smart-platform 无推送日志）、打印正常、状态流转正常 | 若照片未达：确认开关键 `spring.admittance.photo-push-enabled` 生效（Env 端点或日志） |
