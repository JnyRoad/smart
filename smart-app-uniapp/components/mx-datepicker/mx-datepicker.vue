		...
		<view v-if="type!='time'" class="picker-modal">
			...
			<swiper class="picker-modal-body" :circular="true" :duration="200" :skip-hidden-item-layout="true" :current="calendarIndex" @change="onSwiperChange">
				<swiper-item class="picker-calendar" v-for="(calendar,calendarIndex2) in calendars" :key="calendarIndex2">
					<view class="picker-calendar-view" v-for="(week,inx) in weeks" :key="week">
						<view class="picker-calendar-view-item">{{week}}</view>
					</view>
					<view class="picker-calendar-view" v-for="(date,dateIndex1) in calendar" :key="dateIndex1" @click="onSelectDate(date)">
						<!-- 背景样式 -->
		...
		<view v-if="showTimePicker" class="picker">
			<view class="picker-modal picker-time">
				...
				<picker-view class="picker-modal-time" indicator-class="picker-modal-time-item" :value="timeValue" @change="onTimeChange">
					<picker-view-column>
						<block v-for="(v1,i1) in 24" :key="v1">
							<view v-if="i1<10">{{'0'+i1}}时</view>
							<view v-else>{{i1}}时</view>
						</block>
					</picker-view-column>
					<picker-view-column v-if="showApply">
						<block v-for="(item, index) in ['00','30']" :key="index">
							<!-- <view v-if="i2<10">{{'0'+i2}}分</view> -->
							<view>{{item}}分</view>
						</block>
					</picker-view-column>
					<picker-view-column v-if="showMins && !showApply">
						<block v-for="(v2,i2) in 60" :key="v2">
							<view v-if="i2<10">{{'0'+i2}}分</view>
							<view v-else>{{i2}}分</view>
						</block>
					</picker-view-column>
					<picker-view-column v-if="showSeconds">
						<block v-for="(v3,i3) in 60" :key="v3">
							<view v-if="i3<10">{{'0'+i3}}秒</view>
							<view v-else>{{i3}}秒</view>
						</block>
					</picker-view-column>
				</picker-view>
				<view class="picker-modal-footer">
	...
	export default {
		...
		computed: {
			...
			PickerTimeTitle() {
				...
				// 	return DateTools.formatTimeArray(this.timeValue, this.showSeconds);
				// } else {
				// 	return DateTools.formatTimeArray(this.timeValue, this.showMins, this.showSeconds);
				// }
