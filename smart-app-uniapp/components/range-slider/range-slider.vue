...
export default {
	...
	methods: {
		...
		_calculateValues: function(e) {
			...
			var minValue = Math.floor((minLeft / that.MAX_LENGTH) * range + that.min);
			var maxValue = Math.floor((maxLeft / that.MAX_LENGTH) * range + that.min);
		...
		_calculateBlockLeft: function(minValue, maxValue) {
			...
			var range = that.max - that.min;
			var minLeft = Math.floor(((minValue - that.min) / range) * that.MAX_LENGTH);
			var maxLeft = Math.floor(((maxValue - that.min) / range) * that.MAX_LENGTH);
