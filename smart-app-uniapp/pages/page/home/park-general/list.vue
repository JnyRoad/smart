		...
		<block v-for="(item, index) in list" :key="index">
			<view @click="toDetail(item)" class="hlc item mw pr30 pl30 pt20 pb20 bgf mb20 rad10">
				...
				<view class="f1 mh vlb">
					<view class="text2 f30pc">{{ item.generalTitle }}</view>
...
export default {
	...
	methods: {
		async generalList() {
			try {
				const res = await indexInfo.generalList(this.page, this.parkId);
		...
		toDetail(item) {
			if (item.contentLinkType == 1) {
				const link = encodeURIComponent(item.generalUrl)
				uni.navigateTo({
					url: `./detail?link=${link}`
				})
			} else if (item.contentLinkType == 2) {
				uni.navigateTo({
					url: `./detail?generalId=${item.generalId}`
				})
			}
