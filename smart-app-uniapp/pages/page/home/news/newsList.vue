		...
		<block v-for="(item, index) in list" :key="index">
			<view @click="toDetail(item)" class="hlc item mw pr30 pl30 pt20 pb20 bgf mb20 rad10">
				...
				<view class="f1 mh vlb">
					<view class="text2 f30pc">{{ item.newsTitle }}</view>
...
<script>
	...
	export default {
		...
		methods: {
			async newsList() {
				try {
					const res = await indexInfo.newsList(this.page, this.parkId);
			...
			toDetail(item) {
				if (item.contentLinkType == 1) {
					...
					uni.navigateTo({
						url: `./newsDetail?link=${link}`
					})
				} else if (item.contentLinkType == 2) {
					uni.navigateTo({
						url: `./newsDetail?newsId=${item.newsId}`
					})
				}
			}
		},
